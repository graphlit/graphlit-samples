...
async def handle_upload(uploaded_file):
    ...
    if st.session_state['content_id'] is not None:
        with st.spinner('Deleting existing content... Please wait.'):
            await client.delete_content(st.session_state["content_id"] )
    ...
    with st.spinner('Ingesting and analyzing image... Please wait.'):
        ...
        if error_message is not None:
            ...
            st.success(f"Image ingestion and analysis took {duration:.2f} seconds. Finished at {formatted_time} UTC.")
